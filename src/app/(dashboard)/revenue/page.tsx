...
export default function RevenuePage() {
  ...
  return (
    <div className="min-h-screen px-4 md:px-6 py-4 md:py-6 space-y-6" style={{ backgroundColor: '#080910' }}>
      {/* Header */}
      <div className="flex items-center justify-between">
        <div>
          <h1
            className="text-xl font-bold tracking-tight"
            style={{ color: '#e8eaf0', textShadow: '0 0 20px rgba(0,212,255,0.15)' }}
          >
            REVENUE
          </h1>
          <p className="text-[11px] font-mono mt-0.5" style={{ color: '#4a4f65' }}>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard
          icon={DollarSign}
          label="Total Revenue"
          value={loading ? '—' : formatUSD(totalRevenueCents)}
          sub="All-time Gumroad earnings"
          color="#00e5a0"
        />
        <StatCard
          icon={ShoppingBag}
          label="Total Sales"
          value={loading ? '—' : totalSalesCount.toString()}
          sub="Units sold across all products"
          color="#00d4ff"
        />
        <StatCard
          icon={Package}
        ...
        <StatCard
          ...
          value={
            loading || totalSalesCount === 0
              ? '—'
              : formatUSD(totalRevenueCents / totalSalesCount)
          }
          sub="Revenue per unit"
          color="#a78bfa"
        />
      </div>
      ...
      <div
        ...
        style={{ border: '1px solid #1e2030', backgroundColor: '#0d0e14' }}
      >
        <div className="px-5 py-3 md:py-4 flex items-center justify-between" style={{ borderBottom: '1px solid #1e2030' }}>
          <span className="text-[11px] font-mono font-bold tracking-[0.18em] uppercase" style={{ color: '#4a4f65' }}>
            Products
          </span>
          <span className="text-[11px] font-mono" style={{ color: '#2a2d42' }}>
            {products.length} total
          </span>
        </div>

        {loading ? (
          <div className="p-8 text-center text-sm" style={{ color: '#4a4f65' }}>
            Loading...
          </div>
        ) : products.length === 0 ? (
          <div className="p-8 text-center space-y-2">
            <Package size={32} className="mx-auto opacity-20" style={{ color: '#4a4f65' }} />
            <p className="text-sm" style={{ color: '#4a4f65' }}>
              No products yet — Peptide Compass incoming 🚀
            </p>
          </div>
        ) : (
              ...
              <div key={p.id} className="px-5 py-4 flex items-center justify-between">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium" style={{ color: '#c4c9d9' }}>
                      {p.name}
                    </span>
                    <span
                ...
                <div className="flex items-center gap-4">
                  <span className="text-base font-bold" style={{ color: '#00e5a0' }}>
                    {p.formatted_price}
                  </span>
                  {p.short_url && (
                    <a
      ...
      >
        ...
        {loading ? (
          ...
          <div className="p-8 text-center space-y-2">
            ...
            <p className="text-sm" style={{ color: '#4a4f65' }}>
              No sales yet — launch day is coming 💰
            </p>
          </div>
        ) : (
          <div className="divide-y" style={{ borderColor: '#1e2030' }}>
            {recentSales.map((s) => (
              <div key={s.id} className="px-5 py-3 flex items-center justify-between">
                <div className="space-y-0.5">
                  <div className="text-sm" style={{ color: '#c4c9d9' }}>
                    {s.full_name || s.email || 'Anonymous'}
                  </div>
                  <div className="text-[11px]" style={{ color: '#4a4f65' }}>
                    {s.product_name} · {new Date(s.created_at).toLocaleDateString()}
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  {s.refunded && (
                    <span className="text-[9px] font-mono font-bold px-1.5 py-0.5 rounded-full uppercase"
                      style={{ backgroundColor: 'rgba(255,60,60,0.1)', color: '#ff6b6b', border: '1px solid rgba(255,60,60,0.2)' }}>
                      Refunded
                    </span>
                  )}
